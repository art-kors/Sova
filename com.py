...
async def helper(ctx):
    await ctx.message.channel.send('''Полезные ссылки
    https://pymorphy2.readthedocs.io/en/stable/user/grammemes.html
    !analyz - делает морфологический разбор слова.
    Пример: !analyz кит
    Вывод: NOUN,anim,masc,Sgtm,Name sing,nomn
    ----------------------------------
    !in_start_form - Ставит слово в начальную форму.
    Пример: !in_start_form китами
    Вывод: кит
    ----------------------------------
    !in_custom_form - Меняет слово по падежу и числу.
    Пример: !in_custom_form тигр gent plur
    Вывод: тигров
    ----------------------------------
    !is_alive - проверка слово на то одушевленность.
    Пример: !is_alive кит
    Вывод: Живой
    ----------------------------------
    !numerals - проверка слово на то одушевленность.
    Пример: !numerals кит|3
    Вывод: кита
    ----------------------------------
